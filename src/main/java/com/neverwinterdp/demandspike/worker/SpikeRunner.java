...
public class SpikeRunner implements Runnable, Serializable {
  ...
  public void run() {
    if (config.sendPeriod > 0) {
      ...
      try {
        final CountDownLatch localLatch = new CountDownLatch(config.requestsPerThread);
        Client c = null;
        try {
  ...
  void sendContinuous(Client c, final CountDownLatch localLatch) throws InterruptedException {
    ...
    for (long i = 0; i < config.requestsPerThread; i++) {
     // System.out.println(bufferSize.incrementAndGet());
...
//    System.out.println("before while >> requestCounter.get(): "+responseCounter.get()+" >= "+config.requestsPerThread);
//    
//    while(responseCounter.get() < config.requestsPerThread) {
//      System.out.println("inside while >> requestCounter.get(): "+responseCounter.get()+" >= "+config.requestsPerThread);
//     // break;
//    }
    localLatch.countDown();
    Thread.sleep(1000);
