...
public abstract class AbstractApplicationMaster {
    ...
    protected static final Logger LOG = Logger
	    .getLogger(AbstractApplicationMaster.class.getName());
    @Parameter(names = { "-" + Constants.OPT_CONTAINER_MEM,
	    "--" + Constants.OPT_CONTAINER_MEM })
    private int containerMem;

    @Parameter(names = { "-" + Constants.OPT_CONTAINER_COUNT,
	    "--" + Constants.OPT_CONTAINER_COUNT })
    ...
    private String appMasterTrackingUrl = ""; // TODO: What should this really
					      // be?
	...
	ContainerRequest containerReq = new ContainerRequest(capability,
		null /* hosts String[] */, null /* racks String [] */, priority);
	return containerReq;
    }
    ...
    abstract protected List<String> buildCommandList(int startingFrom,
	    int containerCnt, String command);
	...
	public void onContainersCompleted(List<ContainerStatus> statuses) {
	    ...
	    int askAgainCount = totalContainerCount
		    - requestedContainerCount.get();
	...
	public void onContainersAllocated(List<Container> containers) {
		...
		try {
		    ...
		    randomProducerJar = fs.makeQualified(randomProducerJar);
		    jarStat = fs.getFileStatus(randomProducerJar);
		...
		LocalResource randomProducerResource = Records
			.newRecord(LocalResource.class);
		randomProducerResource.setResource(ConverterUtils
			.getYarnUrlFromPath(randomProducerJar));
		randomProducerResource.setSize(jarStat.getLen());
		randomProducerResource.setTimestamp(jarStat
			.getModificationTime());
		randomProducerResource.setType(LocalResourceType.ARCHIVE);
		randomProducerResource
			.setVisibility(LocalResourceVisibility.PUBLIC);

		Map<String, LocalResource> res = new HashMap<String, LocalResource>();
		res.put("lib", randomProducerResource);
		...
		for (int i = 0; i < containerCnt; i++) {
		    ...
		    ctx.setCommands(Collections.singletonList(sb.append(cmdStr)
			    .append(" 1> ")
			    .append(ApplicationConstants.LOG_DIR_EXPANSION_VAR)
			    .append("/stdout").append(" 2> ")
			    .append(ApplicationConstants.LOG_DIR_EXPANSION_VAR)
			    .append("/stderr").toString()));
		    try {
			nodeManager.startContainer(c, ctx);
		    } catch (YarnException e) {
			// TODO: what should I do here? reallocated a new
			// container?
		    } catch (IOException e) {
			// TODO: what should I do here? reallocated a new
			// container?
	...
	// Called when the ResourceManager wants the ApplicationMaster to
	// shutdown
	// for being out of sync etc. The ApplicationMaster should not
	// unregister
	// with the RM unless the ApplicationMaster wants to be the last
	// attempt.
