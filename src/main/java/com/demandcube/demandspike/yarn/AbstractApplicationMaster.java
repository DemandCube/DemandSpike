...
public abstract class AbstractApplicationMaster {
  ...
  private class RMCallbackHandler implements AMRMClientAsync.CallbackHandler {
    ...
    public void onContainersAllocated(List<Container> containers) {
	  ...
	  Path randomProducerJar = new Path("/DemandSpike-standalone.jar");
	  ...
	  LocalResource randomProducerResource = Records.newRecord(LocalResource.class);
	  randomProducerResource.setResource(ConverterUtils.getYarnUrlFromPath(randomProducerJar));
	  randomProducerResource.setSize(jarStat.getLen());
	  randomProducerResource.setTimestamp(jarStat.getModificationTime());
	  randomProducerResource.setType(LocalResourceType.FILE);
	  randomProducerResource.setVisibility(LocalResourceVisibility.PUBLIC);

	  Map<String, LocalResource> res = new HashMap<String, LocalResource>();
	  res.put("random_producer.jar", randomProducerResource); 
      
      for (int i = 0; i < containerCnt; i++) {
	   
        Container c = containers.get(i);
        //String cmdStr = cmdLst.remove(0);
        String cmdStr = "java -jar random_producer.jar";
