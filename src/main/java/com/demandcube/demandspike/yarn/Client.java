...
public class Client {
    ...
    @Parameter(names = { "-" + Constants.OPT_APPNAME,
	    "--" + Constants.OPT_APPNAME })
	...
	// GetNewApplicationResponse can be used to determined resources
	// available.
	...
	amContainer.setLocalResources(Collections.singletonMap("lib",
		appMasterJar));
	...
	appContext.setQueue("default"); // TODO: Need to investigate more on
					// queuing an scheduling.
	...
	sb.append("-Xmx").append(this.applicationMasterMem).append("M")
		.append(" -cp './lib/lib/*' ");
	sb.append(this.applicationMasterClassName).append(" ");
	sb.append("--").append(Constants.OPT_CONTAINER_MEM).append(" ")
		.append(this.containerMem).append(" ");
	sb.append("--").append(Constants.OPT_CONTAINER_COUNT).append(" ")
		.append(this.containerCount).append(" ");
	sb.append("1> ").append(ApplicationConstants.LOG_DIR_EXPANSION_VAR)
		.append("/stdout").append(" ");
	sb.append("2> ").append(ApplicationConstants.LOG_DIR_EXPANSION_VAR)
		.append("/stderr");
	...
	while (true) {
		...
		if (status == FinalApplicationStatus.SUCCEEDED) {
		    ...
		    LOG.info("Application errored out. YarnState="
			    + state.toString() + ", finalStatue="
			    + status.toString());
		    r = false;
		    break;
		}
	...
	classPathEnv.append(Environment.CLASSPATH.$()).append(
	...
	appMasterJar
		.setResource(ConverterUtils.getYarnUrlFromPath(jarHdfsPath));
	...
	LocalResource appMasterJarResource = Records
		.newRecord(LocalResource.class);
	appMasterJarResource.setResource(ConverterUtils
		.getYarnUrlFromPath(appMasterJar));
