...
public class YarnWorkersPerformanceTest extends AbstractMiniClusterUnitTest {
  ...
  public void testYarnContainers() throws Exception {
    ...
    String[] args = {
        ...
        "--app-name", "NeverwinterDP_DemandSpike_App", "--app-container-manager",
        "com.neverwinterdp.demandspike.yarn.master.AsyncDemandSpikeAppMasterContainerManager", "--app-rpc-port",
        "63200", "--app-num-of-worker", "1", "--conf:yarn.resourcemanager.scheduler.address=0.0.0.0:8030",
        "--conf:broker-connect=http://127.0.0.1:7080/message", "--conf:max-duration=3000000",
        "--conf:message-size=1024", "--conf:maxNumOfRequests=10000" };
    ...
    try {
      ...
      /*
       * Configuration conf1 = new Configuration(yarnConf); Path tmpDir1 = new
       * Path("temp"); Path outFile1 = new Path(tmpDir1, "reduce-out");
       * FileSystem fileSys = null;
       * 
       * Text key = new Text(); Text value = new Text(); SequenceFile.Reader
       * reader = null; try { fileSys = FileSystem.get(conf1); reader = new
       * SequenceFile.Reader(fileSys, outFile1, conf1); List<Result> results =
       * new ArrayList<Result>(); while (reader.next(key, value)) {
       ...
       * .toString(resultAggregator.getResult())); } catch (IOException e1) {
       * e1.printStackTrace(); } try { reader.close(); } catch (IOException e) {
       * e.printStackTrace(); }
       */
    } catch (Exception e) {
