...
public class SparknginPerformanceTest {
  ...
  public void testThirtysecondsOneWorker() {
    try {
      ...
      String[] args = { "run", "--target", "http://127.0.0.1:7080/message", "--protocol", "HTTP", "--method", "POST",
          "--input-data", this.data, "--time", "30000" };
  ...
  public void testThirtySecondsTwoWorkers() {
    try {
      ...
      String[] args = { "run", "--target", "http://127.0.0.1:7080/message", "--protocol", "HTTP", "--method", "POST",
          "--input-data", this.data, "--cLevel", "2", "--time", "30000" };
      DemandSpike.main(args);
      server.shutdown();
      printSeparator();
    } catch (Exception e) {
  ...
  public void testWithAllFailues() {
    try {
      ...
      String[] args = { "run", "--target", "http://127.0.0.1:7080/message", "--protocol", "HTTP", "--method", "POST",
          "--input-data", this.data, "--maxRequests", "100000", "--rate", "1000", "--stopOnFailure", "20",
          "--stopOnCondition", "All" };
      long startTime = System.currentTimeMillis();
      DemandSpike.main(args);
      long stopTime = System.currentTimeMillis();
